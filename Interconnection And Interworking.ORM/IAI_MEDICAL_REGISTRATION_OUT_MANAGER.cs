...
namespace InterconnectionAndInterworking.ORM
{
    ...
    /// 门诊操作orm
    /// </summary>
    public class IAI_MEDICAL_REGISTRATION_OUT_MANAGER :
        BaseManager<
            IAI_MEDICAL_REGISTRATION,
            PRPA_IN401001UV02,
            DBNull,
            PRPA_IN900300UV02
            >
    {
        public override IAI_MEDICAL_REGISTRATION Insert(PRPA_IN401001UV02 addEntity)
        {
                ...
                var data = new IAI_MEDICAL_REGISTRATION()
                {
                    ...
                    consultant_id= addEntity.controlActProcess?.subject?.encounterEvent?.consultant?.assignedPerson?.id?.extension,
                    consultant_name= addEntity.controlActProcess?.subject?.encounterEvent?.consultant?.assignedPerson?.assignedPerson?.name?.Value,
                    location_id= addEntity.controlActProcess?.subject?.encounterEvent?.location?.serviceDeliveryLocation?.location?.id?.extension,
                    location_name= addEntity.controlActProcess?.subject?.encounterEvent?.location?.serviceDeliveryLocation?.location?.name?.Value,
                    ...
                    medical_type= EncounterEventTypeCode.CONSULTANT.Value(),
                };
        ...
        public override IAI_MEDICAL_REGISTRATION Select(PRPA_IN900300UV02 queryEntity)
        {
            var result = DBaser.Query<IAI_MEDICAL_REGISTRATION>(
               ...
               new WhereBuilder("medical_type", EncounterEventTypeCode.CONSULTANT.Value()).And("encounterevent_id",
                   queryEntity
                   ?.controlActProcess
        ...
        public override IAI_MEDICAL_REGISTRATION Update(DBNull updateEntity)
        {
