...
namespace InterconnectionAndInterworking.Service
{
    ...
    /// 出院信息查询
    /// </summary>
    class FindEncountersCompletedQuery : AbsHisService<PRPA_IN900300UV02, PRPA_IN900350UV02, PRPA_IN900350UV02>
    {
        ...
        public override void ProcessInfo()
        {
            try
            {
                using (var db = new DataBaser())
                    result = Create<IAI_MEDICAL_REGISTRATION_DIS_MANAGER>(db).Select(requestEntity);
            }
            catch (Exception ex)
            {
                message = ex.Message;
