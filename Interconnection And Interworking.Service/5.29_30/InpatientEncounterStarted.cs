...
namespace InterconnectionAndInterworking.Service
{
    ...
    /// 住院登记
    /// </summary>                                  
    class InpatientEncounterStarted : AbsHisService<PRPA_IN402001UV02, MCCI_IN000002UV01, MCCI_IN000002UV01>
    {
        ...
        public override void ProcessInfo()
        {
            try
            {
                using (var db = new DataBaser())
                    result = Create<IAI_MEDICAL_REGISTRATION_IN_MANAGER>(db).Insert(requestEntity);
            }
            catch (Exception ex)
            {
                message = ex.Message;
