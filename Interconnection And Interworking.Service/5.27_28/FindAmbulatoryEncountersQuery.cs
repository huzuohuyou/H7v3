...
namespace InterconnectionAndInterworking.Service
{
    ...
    /// 门诊就诊查询服务
    /// </summary>
    class FindAmbulatoryEncountersQuery : AbsHisService<PRPA_IN900300UV02, PRPA_IN900350UV02, PRPA_IN900350UV02>
    {
        ...
        public override void ProcessInfo()
        {
            try
            {
                using (var db = new DataBaser())
                    result = Create<IAI_MEDICAL_REGISTRATION_OUT_MANAGER>(db).Select(requestEntity);
            }
            catch (Exception ex)
            {
                message = ex.Message;
